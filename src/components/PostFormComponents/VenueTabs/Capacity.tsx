...
import { CapacityType } from '../../../assets/data/types'

const Capacity: React.FC<TabProps> = ({ nextStep, prevStep }): JSX.Element => {
    const dispatch = useDispatch()
    const capacities_from_redux = useSelector(
        (state: RootState) => state.venueForm.capacity
    )
    const [capacities, setCapacities] = useState<Array<CapacityType>>(
        capacities_from_redux || []
    )

    const next = (): void => {
        dispatch(setForm({ name: 'capacity', value: capacities }))
        nextStep()
    }
    ...
    const handleRoomChange = (e: any, index: number): void => {
        ...
        newCapacities[index].capacity = parseInt(e.target.value)
    ...
    const increment = (index: number): void => {
        ...
        newCapacities[index].capacity += 1
        setCapacities(newCapacities)
    }
